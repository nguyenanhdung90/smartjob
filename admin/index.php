...
if (  class_exists( "ET_Base") ):
	...
	class JE_Job_Edit extends ET_Base{
		...
		public function job_meta_boxes($post){
			...
			<p>
			    <?php global  $current_screen;//print_r($current_screen->action);?>
				<?php if($current_screen->action=="add")
				{ 
			    global $wpdb; $fivesdrafts =$wpdb->get_results( "SELECT ID,display_name FROM wp_post_company where manager !='company' " );
			    ?>
				<label for=""><strong><?php _e('Company', ET_DOMAIN) ?> </strong></label> <br>
				<select class="" id="post_author_override1" name="et_author_name">
					
					<?php foreach($fivesdrafts as $fivesdraft) 
					{
					?>
					<option selected="" value="<?php echo $fivesdraft->ID;?>"><?php echo $fivesdraft->display_name;?></option>
					<?php
					}
					?>
				</select>
				<!-- <input type="text" id="et_company" name="et_author_name" class="et-field" value=""> -->
				<input type="hidden" name="et_author" value="<?php echo $post->post_author ?>"> 
				<?php
				}
				else
				{
					$user = get_user_by( 'id', $post->post_author );//echo $user->roles[0];
					if($user->roles[0]=="company")
					{
				?>					
					<label for=""><strong><?php _e('Company', ET_DOMAIN) ?> </strong></label> <br>
					<!--<input type="text" id="et_company" name="et_author_name" class="et-field" value="">-->
					<label for=""><strong><?php echo $user->display_name; ?></strong></label> 
					<input type="hidden" name="et_author" value="<?php echo $post->post_author ?>"> 
				<?php
				    }
				    else
					{
					global $wpdb; $fivesdrafts =$wpdb->get_results( "SELECT ID,display_name FROM wp_post_company where manager !='company' " );
					$fives =$wpdb->get_results( "SELECT ID,display_name FROM wp_post_company where ID='".$post->company_editor_id."' " );
					$fives=(array) $fives;

				?>
					<label for=""><strong><?php _e('Company', ET_DOMAIN) ?> </strong></label> <br>
				<select class="" id="post_author_override" name="et_author_name">
					<option selected="" value="<?php echo $fives[0]->ID;;?>"><?php echo $fives[0]->display_name;?></option>
					<?php foreach($fivesdrafts as $fivesdraft) 
					{
					?>
					
					<option value="<?php echo $fivesdraft->ID;?>"><?php echo $fivesdraft->display_name;?></option>
					<?php
					}
					?>
				</select>
		...
		public function save_job($post_id){
			...
			//bat dau chinh sua bat  da khi them vao  abng wp_post_company
			//if($current_screen->action=="add")
			//{
			global $wpdb;$wpdb->query("UPDATE wp_posts SET company_editor_id='".$_POST['et_author_name']."'  WHERE ID='".$post_id."' ");	
          //  }
