	...
	if ( $isMobile  ) {
		...
		if(!in_array($filename, array('header-mobile.php' , 'footer-mobile.php')) ) {
			if (is_page_template('page-login.php')){
				...
				$new_template = get_template_directory() . '/mobile/search.php';
			}
		}
	}
	...
	<li data-icon="false" class="list-item">
		...
		<a data-ajax="false" href="{{permalink }}" >
			...
			<p class="list-function">
				...
				<span class="postions" style="font-size:15px">{{author}}</span>
				<#}else{#>	
				<span class="postions" style="font-size:15px">{{name_company_editor}}</span>
				<#}#>
			</p>
			<p class="list-function">
            	<# if ( location != '' ) { #>
					<span class="locations a"><span class="icon" data-icon="@"> </span>{{location}}</span>
				<# } #>			
			</p>
			<p class="list-function">
			<span itemprop="addressLocality" style="font-size:16px">{{mucluong}}:</span>
			<span itemprop="addressLocality" style="color:#F0111B;font-size:16px">{{money_mucluong}}</span>
			</p>
			<p class="list-function"> 
