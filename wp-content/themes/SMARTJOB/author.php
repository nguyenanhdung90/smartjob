<?php get_header(); ?>
    ...
    <div class="heading">
        <div class="main-center">
            ...
            if ($_GET["com_i"]!="") {
                ...
                $rowimage = $rows[0]->logo;
                $rowimage = unserialize($rowimage);
                $rowimage = $rowimage['company-logo'][0];
                ?>
    ...
    <div class="main-column main-left" id="job_list_container">
        <?php if ($_GET["com_i"]!="") { //truong hop la do editor chinh  sua va them
            ...
            <ul class="list-jobs">
                ...
                $wpdb->get_results("SELECT ID FROM wp_posts where post_type ='job'  and post_status='publish' and company_editor_id='" . $_GET["com_i"] . "' ");
                $de = $wpdb->num_rows;
                $dee = $de / POST_PER_PAGE_AUTHOR;
                $_SESSION['coun_page'] = (int)$dee + 1;
                unset($dee);
                if ($_GET["page"] == "") $count_page = 1; else $count_page = $_GET["page"];
                $start = POST_PER_PAGE_AUTHOR * ($count_page - 1);
                $end = POST_PER_PAGE_AUTHOR * $count_page;
                $string_query = "SELECT * FROM wp_posts  where post_type ='job' and post_status='publish' and company_editor_id='" . $_GET["com_i"] . "' ORDER BY post_date DESC   LIMIT " . $end . " OFFSET " . $start . "  ";
                ...
                foreach ($fivesdrafts as $fivesdraft) {
                    ...
                    <li class="job-item" style="display: block;">
                        ...
                        <div class="content null">
                            <a target="_blank"
                               href="<?php bloginfo('url'); ?>/job/<?php echo $fivesdraft->post_name; ?>"
                               class="title-link title"> <?php echo $fivesdraft->post_title; ?></a>
                            <a href="<?php bloginfo('url'); ?>/job/<?php echo $fivesdraft->post_name; ?>"
                               class="title-link title" target="_blank"><span data-icon="R" class="icon"></span></a>
                            ...
                            <div class="desc f-left-all">
                                <div class="cat company_name c">
                                    <a target="_blank"
                                       href="<?php echo $company['post_url'] . '?com_i=' . $_GET["com_i"]; ?>"><?php echo $rows[0]->display_name; ?></a>
                                </div>
                                    ...
                                    foreach ($tags_ as $tag_) {
                                        ...
                                        foreach ($tags_name as $tag_name) {
                                            ...
                                            <a target="_blank" rel="tag"
                                               href="<?php bloginfo('url'); ?>/job-type/<?php echo $tag_name->name; ?>"><?php echo $tag_name->name; ?></a>
                                        <?php
                                        }
                                    }
                                    ?>
                            ...
                            ?>
                                ...
                                <div><span itemprop="jobLocation" itemscope="" itemtype="" class="ob-location"><span
                                            class="icon">Salary:</span>
							<span style="color:#f0111b" class="job-location">
							<?php
                            ...
                            <div class="decription_smartjob">
                                <?php $bien = mb_substr($fivesdraft->post_content, 0, 300);
                            ...
                            <div class="decription_smartjob">
                                ...
                                foreach ($tags_ as $tag_) {
                                    ...
                                    foreach ($tags_name as $tag_name) {
                                        ?>
                                        <a class="tag_smartjob_home"
                                           href="<?php bloginfo('url'); ?>/?s=<?php echo $tag_name->name; ?> "
                                           target="_blank"> <?php echo $tag_name->name; ?></a>
                                    <?php
                                    }
                                }
                                ?>
        ...
        <?php
        } else { // truong hop la cong ty company
            //$rows = $wpdb->get_results("SELECT decription FROM wp_post_company where users_id ='" . $company_id . "' ");
            ?>
            <ul class="list-jobs" style="padding-top:14px;padding-bottom:14px;text-align:justify">
                <?php //echo $rows[0]->decription; ?>
                <?php 
					$company		= et_create_companies_response($company_id );
    ...
    if ($_GET["com_i"]!="") {
        ...
        <div id="sidebar-company" class="second-column widget-area sortable ui-sortable">
            <aside class="widget company-profile bg-grey-widget margin-top15 %2$s" id="%1$s">
                <div class="thumbs">
                    <a class="thumb" title="View posted jobs by  <?php echo $rows[0]->display_name; ?>" href=""
                       id="job_author_thumb">
                        <img id="company_logo_thumb" src="<?php echo $rowimage; ?>">
                    </a>
                </div>
                <div class="title company_name">
                    <a title="View jobs posted <?php echo $rows[0]->display_name; ?> " href=""
                       class="name job_author_link" id="job_author_name">
                        <?php echo $rows[0]->display_name; ?>
                    </a>
                </div>
                <div class="info icon-default">
                    <a href=" <?php echo $rows[0]->user_url; ?>	" rel="nofollow" target="_blank"
                       id="job_author_url"> <?php echo $rows[0]->user_url; ?></a>
