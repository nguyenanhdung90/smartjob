...
class JE_Resume extends ET_Base{
    ...
    static public function insert($args){
        ...
        $args = wp_parse_args( $args, array(
            ...
            'post_status'   => ($pending_resume) ? 'publish' : 'pending',
        ...
        try {
            ...
            if (!is_wp_error( $app ) && is_numeric($app)){
				...
				if($role_user_post[0] !="company"){
					global $wpdb;
					$fivesdrafts =$wpdb->get_results( "SELECT user_email,display_name FROM wp_post_company WHERE ID= '".$job->company_editor_id."' " );
                    ...
                    $this->send_application_mail_editor($args, $template= array(), $attach_id, $email_company_editor, $display_name_editor);
				}
				else
				{
        ...
        */
        ...
        $mail_to_employee = array(
            ...
            'headers'   => 'MIME-Version: 1.0' . "\r\n" .
                        'Content-type: text/html; charset=utf-8' . "\r\n" .
                        "From: ".get_option('blogname')." < ".get_option('admin_email') ."> \r\n".
						"Bcc: ".get_option('blogname')."< hr@dcv.vn> \r\n"
            );
        $attachment = '';
        if($attach_id)
        ...
        wp_mail( $mail_to_employee['to'], $mail_to_employee['subject'], $mail_to_employee['message'], $mail_to_employee['headers'], $attachment );
    }
    private function send_application_mail_editor($args, $template_args = array(), $attach_id = false, $email_company_editor, $display_name_editor){
        $job            = get_post($args['job_id']);
        $company        = get_userdata( $job->post_author );
        $jobseeker      = get_userdata( $args['jobseeker_id'] );
        $resume         = get_post( $args['resume_id'] );
        $mail_template  = JE_Resumes_Mailing::get_instance();
        $company_vari   = array(
            'display_name'  => $display_name_editor,
            'job_title'     => $job->post_title,
            'seeker_name'   => $args['emp_name'],
            'seeker_mail'   => $args['emp_email'],
            'seeker_note'   => $args['apply_note'],
        ...
        */
        ...
        if($attach_id)
            $attachment = array(get_attached_file($attach_id));
        wp_mail( $email_company_editor, $mail_to_company['subject'], $mail_to_company['message'], $header ='', $attachment ); 
        wp_mail( $mail_to_employee['to'], $mail_to_employee['subject'], $mail_to_employee['message'], $mail_to_employee['headers'], $attachment);
    }
}
