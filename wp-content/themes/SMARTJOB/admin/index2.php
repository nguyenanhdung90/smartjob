...
if (  class_exists( "ET_Base") ):
	...
	class JE_Job_Edit extends ET_Base{
		...
		public function job_meta_boxes($post){
			...
			<p>
				...
				<select class="" id="post_author_override" name="et_author_name">
					...
					{
					?>
					<option selected="selected" value="<?php echo $fivesdraft->ID;?>"><?php echo $fivesdraft->display_name;?></option>
					<?php
					}
					?>
				...
				?>
					<label for=""><strong><?php _e('Company', ET_DOMAIN) ?> </strong></label> <br>
					<input type="text" id="et_company" name="et_author_name" class="et-field" value="<?php foreach($fivesdrafts as $fivesdraft) {echo $fivesdraft->display_name;}$wpdb->flush(); ?>">
					<input type="hidden" name="et_author" value="<?php echo $post->post_author ?>"> 
				<?php
					}
				?>
		...
		public function save_job($post_id){
			...
			//global $wpdb;$wpdb->query("INSERT INTO   wp_post_company (display_name) VALUES ('company')");	
