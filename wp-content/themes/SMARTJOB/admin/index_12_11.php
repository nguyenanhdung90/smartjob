		...
		public function job_meta_boxes($post){
			...
			<p>
				...
				<?php
					...
					{
					global $wpdb; $fivesdrafts =$wpdb->get_results( "SELECT display_name FROM wp_post_company where ID='".$post->company_editor_id."' " );
				?>
					<label for=""><strong><?php _e('Company', ET_DOMAIN) ?> </strong></label> <br>
					<!--<input type="text" id="et_company" name="et_author_name" class="et-field" value="">-->
					<label for=""><strong><?php foreach($fivesdrafts as $fivesdraft) {echo $fivesdraft->display_name;}$wpdb->flush(); ?></strong></label> 
					<input type="hidden" name="et_author" value="<?php echo $post->post_author ?>"> 
				<?php
					}
				?>
